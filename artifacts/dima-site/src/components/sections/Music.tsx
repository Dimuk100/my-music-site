    ...
    >
      <div className="flex items-center justify-between mb-4 sm:mb-8">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-2xl sm:text-3xl font-display font-bold text-white">{t.music.title}</h2>
        </div>
        ...
        {songs.map((song) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className={`text-sm sm:text-base font-medium ${song.active ? "text-white" : "text-white/80"}`}>
                  {song.title}
                </h4>
                <p className="text-xs text-white/40 mt-0.5">Dima Vardzelashvili</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs sm:text-sm font-mono text-white/50">{song.duration}</span>
              <button className="text-white/30 hover:text-red-400 transition-colors">
                <Heart className={`w-4 h-4 sm:w-5 sm:h-5 ${song.active ? "fill-white/10" : ""}`} />
              </button>
            </div>
          </div>
        ))}
